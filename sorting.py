...
pygame.mixer.init()
def audio():
    pygame.mixer.music.load("algo.mp3")
...
def drawData(data,colorArray):
    ...
    for i , height in enumerate(normalised_data):
        x0 = i*x_width + offset + spacing_bat_rect
        y0 = canvas_height - height * 400   # we have multiples 400 because we will normalised our values with one 
...
algo_menu = ttk.Combobox(root,width = 15,font = ('new roman',19,'italic bold'),textvariable = selected_algorithm,values=['Bubble Sort','Merge Sort','Quick Sort','Selection Sort','Heap Sort'])
